...
DEVICE = LLM_DEVICE
DEVICE_ID = "0"
MLU_DEVICE = f"{DEVICE}:{DEVICE_ID}" if DEVICE_ID else DEVICE
...
def torch_gc():
    if torch.mlu.is_available():
        with torch.mlu.device(MLU_DEVICE):
            torch.mlu.empty_cache()
            #torch.mlu.ipc_collect()


def auto_configure_device_map(num_mlus: int) -> Dict[str, int]:
    num_trans_layers = 28
    per_mlu_layers = 30 / num_mlus
    ...
    for i in range(num_trans_layers):
        if used >= per_mlu_layers:
            mlu_target += 1
            used = 0
        assert mlu_target < num_mlus
        device_map[f'transformer.layers.{i}'] = mlu_target
        used += 1
...
class ChatLLM(LLM):
    ...
    def _call(self, prompt: str, stop: Optional[List[str]] = None) -> str:
        ...
        if self.model_type == 'vicuna':
            ...
            output_ids = self.model.generate(
                torch.as_tensor(inputs.input_ids).mlu(),
                do_sample=True,
    ...
    def load_llm(self,
        ...
        if 'chatglm' in self.model_name_or_path.lower():
            ...
            if torch.mlu.is_available() and llm_device.lower().startswith("mlu"):

                num_mlus = torch.mlu.device_count()
                if num_mlus < 2 and device_map is None:
                    ...
                    if device_map is None:
                        device_map = auto_configure_device_map(num_mlus)
